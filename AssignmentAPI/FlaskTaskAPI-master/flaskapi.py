...
@app.route('/api/projects/get/byuserid')
def fetch_projects_by_user_id():
    try:
        userId = request.args.get('id')
        print(userId)
        conn = get_db_connection()
        return SQL_to_json(conn.execute('SELECT * FROM projects WHERE user_id=?', (userId,)).fetchall())
    except:
        return jsonify({"Error" : "An error occurred while fetching projects by user ID!"})
    finally:
        print(f"Finalising /api/projects/get/byuserid")
        conn.close()
...
def update_project():
    try:
        ...
        conn = get_db_connection()
        conn.execute('UPDATE projects SET name=?, description=?, start_date=?, end_date=?, user_id=? WHERE id=?',(name, description, start_date, end_date, user_id, id))
        return
        SQL_to_json(conn.execute('SELECT * FROM projects WHERE id=?', (id,)).fetchall())
    except:
        return jsonify({"Add_Success": False})
    finally:
...
def fetch_task_by_id():
    try:
        projectId = request.args.get('id')
        conn = get_db_connection()
        return SQL_to_json(conn.execute('SELECT * FROM tasks WHERE project_id=?', (taskId,)).fetchall())
    except:
...
@app.route('/api/tasks/get/byprojectid')
def fetch_tasks_by_project_id():
    try:
        projectid = request.args.get('id')
        print(projectid)
        conn = get_db_connection()
        return SQL_to_json(conn.execute('SELECT * FROM tasks WHERE project_id=?', (projectid,)).fetchall())
    except:
        return jsonify({"Error" : "An error occurred while fetching tasks by project ID!"})
    finally:
        print(f"Finalising /api/tasks/get/byprojectid")
        conn.close()
